...
class colourIdentifier(Node):
    ...
    def callback(self, data):
        ...
        try:
            ...
            hsv_red_first_lower = np.array([175 - self.sensitivity, 100, 100])
            hsv_red_first_upper = np.array([180, 255, 255])
            hsv_red_second_lower = np.array([0, 100, 100])
            hsv_red_second_upper = np.array([5 + self.sensitivity, 255, 255])
            ...
            red_first_mask = cv2.inRange(Hsv_image, hsv_red_first_lower, hsv_red_first_upper)
            red_second_mask = cv2.inRange(Hsv_image, hsv_red_second_lower, hsv_red_second_upper)
            red_mask = cv2.bitwise_or(red_first_mask, red_second_mask)
            ...
            red_blue_green_mask = cv2.bitwise_and(image, image, mask = combined_mask)
        except CvBridgeError as e:
            print(e)

        #Show the resultant images you have created. You can show all of them or just the end result if you wish to.
        cv2.imshow('Mask', red_blue_green_mask)
        cv2.namedWindow('camera_feed', cv2.WINDOW_NORMAL)
        cv2.imshow('camera_feed', red_blue_green_mask)
        cv2.resizeWindow('camera_feed', 320, 240)
        cv2.waitKey(3)
